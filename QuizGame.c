// These lines are preprocessors that import the libraries. 
#include <stdio.h>
#include <string.h>
#include <stdbool.h> 
#include "Quiz.c" // References the structures from a different file 

int main(void) { 

    // these lines intialize the structures by defaulting to a null value for each category of quesions 
    ...
    while (isDone == 1) { // this loop starts the quiz by asking category 
        ...
        if (category == 1) { // if the category is selected then it starts the questions from this category 
            ...
            for (j = 0; j < 5; j++) { // this is the for loop that goes over each question
                printf("%s", &CyberSecurity.questions.Q[j].index.Q); // prints the category if chosen
                scanf_s("%c", &answer);// The program execution wasnt stopping at the first scanf so i used 2 
                scanf_s("%c", &answer); //  user input
